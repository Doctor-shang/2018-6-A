var oSj = 5000;// 轮播间隔
        ...
        $(".but-left").click(function(){
            ...
            i--;
            if(i == -1){
                $('.banner-box .img').css('left',-(imgNum-1)*1200);//用CSS进行图片位置变换，达到无缝拼接效果
                i = imgNum-2;
            }
            oImg.stop().animate({left:-i*1200},500);//动画效果
            clearInterval(oTime);
        ...
        $(".banner-box .li li").hover(function() {
            ...
            i=index;
            oImg.stop().animate({left:-index*1200},500);//动画效果
            bar.stop().css('width',0);
        ...
        function int(){
            ...
            barAniMate();//进度条函数动画效果
            clearInterval(oTime);
            oTime = setInterval(function(){int();},oSj);
            if(i == imgNum-1){
